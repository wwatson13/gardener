...
func (c *AddToManagerConfig) AddToManager(mgr manager.Manager) ([]admissionregistrationv1beta1.MutatingWebhook, []admissionregistrationv1beta1.MutatingWebhook, error) {
	...
	if c.Server.ServicePort > 0 {
		port = c.Server.ServicePort
	}
	...
	seedWebhooks, shootWebhooks, err := extensionswebhook.RegisterWebhooks(ctx, mgr, c.Server.Namespace, c.serverName, port, c.Server.Mode, c.Server.URL, caBundle, webhooks)
	if err != nil {
